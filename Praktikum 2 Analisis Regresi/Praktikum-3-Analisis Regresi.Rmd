---
title: "Praktikum 3 Analisis Regresi"
...
```{r}
library(readxl)
dt <- read_xlsx("C:\\Users\\62878\\OneDrive\\Desktop\\Documents\\Analisis Regresi\\data analisis regresi\\DATA PRAKTIKUM ANREG.xlsx")
dt
```

```{r}
y <- dt$"PDRB"
x <- dt$"Persentase Penduduk yang Memiliki Jaminan Kesehatan"
n <- length(dt)
```
...
Oleh karena simpangan baku merupakan akar kuadrat dari ragam, maka nilai dugaan galat baku model yang kita bentuk adalah $S_e = \sqrt{2.831974} \approx  65315.17$
...
Sebelumnya, melalui fungsi `lm` dan perhitungan manual yang sudah dibahas pada pertemuan sebelumnya, kita sudah mendapatkan nilai dugaan parameter β0 dan β1 sebesar 5038 dan 1066 . Kedua nilai dugaan parameter tersebut dapat kita definisikan sebagai berikut.
...
Sehigga dapat disimpulkan bahwa pada taraf signifikansi 5%, selang kepercayaan untuk parameter $β_0$ adalah [-248174.1 ,  258250.4 ] dan untuk parameter $β_1$ adalah [-2029.851 ,  4160.887]
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan rataan nilai $Y$ ketika nilai x=10 adalah 15693.32. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai dugaan rataan $Y$ ketika nilai x=10 berada dalam selang -154645.7  hingga 186032.4.
...
Berdasarkan output sintaks di atas, diketahui bahwa dugaan nilai individu Y ketika nilai x=10 adalah 15693.32. Selain itu, diindikasikan juga bahwa dalam taraf kepercayaan 95%, diyakini bahwa nilai amatan individu Y ketika nilai x=10 berada dalam selang -200090 hingga 231476.6.